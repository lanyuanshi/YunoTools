...
- 变更重点：
  ...
  - 音乐播放器改为网易云音乐 + 酷狗音乐多源公开搜索，自动过滤不可播放结果。
...
YunoTools 是一个基于原生 Kotlin + Android SDK 的 Android 工具箱应用，面向日常处理、媒体播放、图像处理和轻量信息查询场景。当前版本重点修复了指南针海拔/天气的可见性，并把在线音乐改成可用的多源公开搜索方案，搜索阶段会探测播放直链，只展示可播放结果。
...
- 音乐播放器改为网易云音乐 + 酷狗音乐多源公开搜索，并在播放前过滤不可播结果。
