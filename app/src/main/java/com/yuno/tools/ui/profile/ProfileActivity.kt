...
class ProfileActivity : AppCompatActivity() {
    ...
    private fun confirmClear() {
        ...
        AlertDialog.Builder(this)
            .setTitle("清空历史记录")
            .setMessage("确定要清空所有解析历史吗？")
            .setNegativeButton("取消", null)
            .setPositiveButton("清空") { _, _ ->
                ParseHistoryStore.clear(this)
                loadHistory()
            }
            .show()
    }
    ...
    private fun openHistory(item: ParseHistoryItem) {
        val intent = if (item.result.isImageSet) {
    ...
    private class HistoryAdapter(
        ...
        private val onClick: (ParseHistoryItem) -> Unit
    ) : RecyclerView.Adapter<HistoryAdapter.VH>() {
        private val items = mutableListOf<ParseHistoryItem>()
        fun submit(newItems: List<ParseHistoryItem>) {
            items.clear(); items.addAll(newItems); notifyDataSetChanged()
        }
        override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH =
            VH(LayoutInflater.from(parent.context).inflate(R.layout.item_parse_history, parent, false))
        ...
        class VH(itemView: View) : RecyclerView.ViewHolder(itemView) {
            ...
            fun bind(item: ParseHistoryItem, timeFormat: SimpleDateFormat, onClick: (ParseHistoryItem) -> Unit) {
                ...
                Glide.with(cover).load(item.coverUrl.ifBlank { item.result.images.firstOrNull() })
                    .placeholder(R.drawable.bg_circle_blue_light).error(R.drawable.bg_circle_blue_light).centerCrop().into(cover)
