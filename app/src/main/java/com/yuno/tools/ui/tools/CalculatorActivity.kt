...
class CalculatorActivity : AppCompatActivity() {
    ...
    private fun hero() = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
        background = gradient("#2563EB", "#7C3AED", 28)
        setPadding(dp(18), dp(18), dp(18), dp(18))
        layoutParams = LinearLayout.LayoutParams(-1, -2).apply { bottomMargin = dp(14) }
        addView(TextView(context).apply { text = "全能计算器"; textSize = 25f; typeface = Typeface.DEFAULT_BOLD; setTextColor(Color.WHITE) })
        addView(TextView(context).apply { text = "标准、科学、程序员、换算、日期、房贷、个税、BMI，一页整合。"; textSize = 13.5f; setTextColor(Color.argb(232,255,255,255)); setPadding(0, dp(8), 0, 0) })
    }
    ...
    private fun showProgrammer() {
        ...
        panel.addView(card("进制转换", input, out, "转换", "#2563EB") {
            val n = input.text.toString().trim().toLongOrNull() ?: return@card toast("请输入整数")
        ...
        panel.addView(card("位运算", bitA, bitB, bitOut, "计算", "#7C3AED") {
            val a = bitA.text.toString().trim().toLongOrNull() ?: return@card toast("请输入 A")
            val b = bitB.text.toString().trim().toLongOrNull() ?: return@card toast("请输入 B")
    ...
    private fun showMortgage() {
        ...
        panel.addView(card("房贷计算", amount, years, rate, out, "计算", "#2563EB") {
            val principal = (amount.numOrNull() ?: return@card toast("请输入贷款总额")) * 10000
            val months = ((years.numOrNull() ?: return@card toast("请输入年限")) * 12).roundToInt()
            val monthRate = (rate.numOrNull() ?: return@card toast("请输入利率")) / 100 / 12
            val equalPayment = principal * monthRate * (1 + monthRate).pow(months) / ((1 + monthRate).pow(months) - 1)
    ...
    private fun showTax() {
        ...
        panel.addView(card("个人所得税", salary, social, out, "计算", "#7C3AED") {
            val taxable = (salary.numOrNull() ?: return@card toast("请输入收入")) - (social.numOrNull() ?: 0.0) - 5000
            val tax = monthlyTax(taxable.coerceAtLeast(0.0))
            out.text = "应纳税所得额：${money(taxable.coerceAtLeast(0.0))}\n预估个税：${money(tax)}\n税后收入：${money((salary.numOrNull() ?: 0.0) - tax)}"
        })
    }
    ...
    private fun showBmi() {
        ...
        panel.addView(card("BMI 与健康体重", height, weight, out, "计算", "#10B981") {
            val h = (height.numOrNull() ?: return@card toast("请输入身高")) / 100
            val w = weight.numOrNull() ?: return@card toast("请输入体重")
            val bmi = w / (h * h)
    ...
    private fun grid(keys: List<String>) = GridLayout(this).apply {
        columnCount = 4
        keys.forEach { key -> addView(calcKey(key), GridLayout.LayoutParams().apply { width = 0; height = dp(54); columnSpec = GridLayout.spec(GridLayout.UNDEFINED, 1f); setMargins(dp(4), dp(4), dp(4), dp(4)) }) }
    }
    ...
    private fun addConvert(title: String, unit: String, targets: List<Pair<String, Double>>, custom: ((Double) -> String)? = null) {
        ...
        panel.addView(card(title, input, out, "换算", "#2563EB") {
            val v = input.numOrNull() ?: return@card toast("请输入数字")
            out.text = custom?.invoke(v) ?: targets.joinToString("\n") { "${it.first}：${fmt(v * it.second)}" }
        })
    }

    private fun card(title: String, vararg views: android.view.View, buttonText: String, color: String, action: () -> Unit): LinearLayout = LinearLayout(this).apply {
        orientation = LinearLayout.VERTICAL
        background = rounded("#FFFFFF", 24)
    ...
    private fun fmt(v: Double): String = if (v.isNaN() || v.isInfinite()) "--" else if (abs(v - v.roundToInt()) < 0.0000001) v.roundToInt().toString() else "%.6f".format(v).trimEnd('0').trimEnd('.')
